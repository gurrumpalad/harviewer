...
$(document).ready(function(){
    ...
    if ($('.js-ajaxForm').length > 0 && $('.js-ajaxListener').length > 0) {
        $('.js-ajaxListener').on('change', '.js-ajaxForm .js-checkFile', function(){
            $(this).nextAll('.js-checkFile, .js-viewFile').remove();
                ...
                if (tabObj) {
                    try {
                        $.getJSON($(this).val(), function(input){
                            ...
                            fileItems.append('<div class="FileItem js-fileItem">' + $(elem).val() + '<div class="FileItem__remove js-fileRemove">Удалить</div></div>');
                        });
                    } catch (e) {
                        console.log(e);
                    }
